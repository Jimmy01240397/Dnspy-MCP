...
public static class AsmFileTools
{
    ...
    public static object AsmOpen(Workspace ws, string asmPath) => ToolGuard.Run(() =>
    {
    ...
    public static object AsmClose(Workspace ws, string asmPath)
        => ToolGuard.Run(() => (object)new { closed = ws.Close(asmPath), current = ws.Current });
    ...
    public static object AsmList(Workspace ws) => ToolGuard.Run(() =>
        (object)ws.All.Select(a => new {
            path = a.Path,
            name = a.Module.Name?.String,
            types = a.Module.GetTypes().Count(),
    ...
    public static object AsmCurrent(Workspace ws) => ToolGuard.Run(() => (object)new { current = ws.Current });
    ...
    public static object AsmSwitch(Workspace ws, string asmPath) => ToolGuard.Run(() =>
    {
        var a = ws.Switch(asmPath);
        return (object)new { current = a.Path };
    });
        ...
        => ToolGuard.Run(() =>
    {
        ...
        return (object)Paging.Page(filtered, offset, max);
    });
        ...
        => ToolGuard.Run(() =>
    {
        ...
        return (object)Paging.Page(rows, offset, max);
    });
        ...
        => ToolGuard.Run(() =>
    {
        ...
        return (object)Paging.ClampText(full, offsetChars, maxChars);
    });
        ...
        => ToolGuard.Run(() =>
    {
        ...
        var m = overloads[overloadIndex];
        if (!m.HasBody) return (object)new { total = 0, offset, returned = 0, truncated = false, items = Array.Empty<object>() };
        ...
        => ToolGuard.Run(() =>
    {
        ...
        if (!md.HasBody) return (object)new { total = 0, offset, returned = 0, truncated = false, items = Array.Empty<object>() };
